 ...
 *   ordered: {contractKey, number, orderRef, qty, sum};
 *   received: {contractKey, zayavka, qty, sum};
...
export function reconcileByZayavka({ ordered, shipped, received }) {
  ...
  for (const l of received) {
    ...
    // Если номер заявки из БУХ напрямую не совпал с заявкой УНФ,
    // но у контракта в УНФ ровно одна заявка,
    // соответствие можно считать однозначным.
    if (!m.has(zk)) {
  ...
  // В первую очередь сшиваем по ссылке на ЗаказПокупателя.
  for (const l of shipped) {
    ...
    if (!zk) {
      ...
      // Накладная есть, но ссылка на конкретную заявку не найдена.
      zk = l.contractKey + '#—';
    }
