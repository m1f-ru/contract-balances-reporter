...
export class UnfRepo {
  ...
  async naklLines(contractorRefs = null) {
    ...
    for (const h of kept) headByRef.set(h.Ref_Key, { number: h.Number, date: h.Date, contractKey: contractKey(h.ОснованиеПечати ?? ''), order: h.Заказ });
    ...
    for (const l of lines) {
      ...
      if (!h) continue;
      out.push({ doc: l.Ref_Key, number: h.number, date: h.date, contractKey: h.contractKey, order: h.order, nom: l.Номенклатура_Key, qty: Number(l.Количество) || 0, sum: Number(l.Сумма) || 0 });
    }
    return out;
  }
  ...
  async orderLines(contractorRefs = null) {
    ...
    const kept = contractorRefs ? heads.filter(h => contractorRefs.has(String(h.Контрагент_Key))) : heads;
    const headByRef = new Map();
    for (const h of kept) headByRef.set(h.Ref_Key, { number: h.Number, date: h.Date, contractKey: contractKey(h.ОснованиеПечати ?? '') });

    const lines = await this.client.getCollection(ORDER_LINES, {
      '$format': 'json', '$select': 'Ref_Key,Количество,Сумма',
    });
    const out = [];
    for (const l of lines) {
      const h = headByRef.get(l.Ref_Key);
      if (!h) continue;
      out.push({ doc: l.Ref_Key, orderRef: l.Ref_Key, number: h.number, date: h.date, contractKey: h.contractKey, qty: Number(l.Количество) || 0, sum: Number(l.Сумма) || 0 });
    }
    return out;
  }
}
